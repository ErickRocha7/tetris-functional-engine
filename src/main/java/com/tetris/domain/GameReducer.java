...
public final class GameReducer {
    ...
    /**
     * Função matemática pura central: NovoEstado = reduce(EstadoAtual, Evento).
     */
    public static GameState reduce(GameState current, GameEvent event) {
        // Bloqueio declarativo de inputs se o jogo estiver encerrado
        if (current.status() == GameStatus.GAME_OVER) {
            return current;
        }

        // Bloqueio declarativo de comandos do jogador durante animações da UI
        ...
        return switch (event) {
            ...
            case GameEvent.LineAnimationEnd() -> completeLineClearing(current);
        };
    }
    ...
    private static GameState handleMove(GameState state, int dRow, int dCol) {
        ...
        if (moved == state.activePiece())
            return state;
        return new GameState(state.board(), moved, state.nextQueue(), state.score(), state.linesCleared(),
                state.status());
    }
    ...
    private static GameState handleRotation(GameState state) {
        ...
        if (rotated == state.activePiece())
            return state;
        return new GameState(state.board(), rotated, state.nextQueue(), state.score(), state.linesCleared(),
                state.status());
    }
    ...
    private static GameState handleGravity(GameState state) {
        ...
        if (state.board().isValidPosition(movedDown)) {
            return new GameState(state.board(), movedDown, state.nextQueue(), state.score(), state.linesCleared(),
                    state.status());
        }

        // Lock-in: A peça colidiu embaixo e se funde de forma estável ao tabuleiro
        ...
        if (updatedQueue.isEmpty())
            updatedQueue.add(Tetromino.Shape.T); // Abastecimento contínuo simplificado

        Tetromino newActivePiece = new Tetromino(nextShape, new MatrixPosition(0, 4), Tetromino.Orientation.NORTH);

        return new GameState(mergedBoard, newActivePiece, List.copyOf(updatedQueue), state.score(),
                state.linesCleared(), nextStatus);
    }
    ...
    private static GameState handleHardDrop(GameState state) {
        ...
        while (step.board().isValidPosition(current.move(1, 0))) {
            ...
            step = new GameState(step.board(), current, step.nextQueue(), step.score() + 2, step.linesCleared(),
                    step.status());
        }
    ...
    private static GameState completeLineClearing(GameState state) {
        ...
        while (newMatrixList.size() < originalMat.length) {
            newMatrixList.add(0, new Tetromino.Shape[originalMat[0].length]);
        }

        long newScore = state.score() + (clearedCount * 100L);
        long newLines = state.linesCleared() + clearedCount;
        Board finalBoard = new Board(newMatrixList.toArray(new Tetromino.Shape[0][0]));

        return new GameState(finalBoard, state.activePiece(), state.nextQueue(), newScore, newLines,
                GameStatus.PLAYING);
    }
